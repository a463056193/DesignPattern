package com.example.designpattern.creational.simpleFactoryPattern.first;

/**
 * Sunny软件公司欲基于Java语言开发一套图表库
 * 该图表库可以为应用系统提供各种不同外观的图表
 * 例如柱状图、饼状图、折线图等
 * Sunny软件公司图表库设计人员希望为应用系统开发人员提供一套灵活易用的图表库，而且可以较为方便地对图表库进行扩展
 * 以便能够在将来增加一些新类型的图表
 */
public class Chart {
    //初始设计方案，将所有图表的实现代码封装在一个Chart类中
    private String type;
    ...
    // (2) Chart类的职责过重，它负责初始化和显示所有的图表对象
    ...
    // 例如设置柱状图的颜色、高度等，如果在Chart类的构造函数中没有提供一个默认设置，那就只能由客户端来完成初始设置
    // 这些代码在每次创建Chart对象时都会出现，导致代码的重复


    /*版权声明：本文为CSDN博主「LoveLion」的原创文章，遵循CC 4.0 BY-SA版权协议，转载请附上原文出处链接及本声明。
    原文链接：https://blog.csdn.net/lovelion/article/details/9300337*/
